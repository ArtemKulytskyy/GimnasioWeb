import {Link} from 'react-router-dom';
import BtnIniciarSesion from './common/BtnIniciarSesion';
import BtnRegistrarme from './common/BtnRegistrarme';


function IniciarSesionForm() {
  return (
    <div className="modal fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div className="modal-dialog">
        <div className="modal-content">
        <div className="modal-header">
            <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div className="modal-body">
                <div className="container  d-flex align-items-center justify-content-center py-4">
                <div className="card shadow-sm border rounded-4 w-100" style={{maxWidth: "520px"}}>
                    <div className="card-body p-4 p-md-5">

                    <h1 className="h3 fw-bold mb-2 bebas-neue color-gris">Accede a FITNESS PARK</h1>
                    <p className="text-secondary mb-4">
                        Inicia sesión si ya tienes cuenta,<br />
                        o regístrate en un minuto.
                    </p>

                    <BtnRegistrarme extraClass='border-dark col-md-12 col-12'/>

                    <div className="divider mb-4">
                        <span>o</span>
                    </div>

                    <h2 className="h3 fw-bold mb-3 bebas-neue color-gris">Iniciar Sesión</h2>

                    <form>
                        <div className="mb-3">
                        <input
                            type="email"
                            className="form-control form-control-lg rounded-4"
                            placeholder="Email"
                            aria-label="Email"
                        />
                        </div>

                        <div className="mb-3">
                        <input
                            type="password"
                            className="form-control form-control-lg rounded-4"
                            placeholder="Contraseña"
                            aria-label="Contraseña"
                        />
                        </div>

                        <div className="form-check d-flex align-items-center gap-2 mb-3">
                        <input className="form-check-input" type="checkbox" id="recordarme" />
                        <label className="form-check-label fw-semibold" for="recordarme">
                            Recordarme &gt;
                        </label>
                        </div>

                        <BtnIniciarSesion extraClass='border-dark col-md-12 col-12'/>
                    </form>

                    </div>
                </div>
                </div>
        </div>
        </div>
    </div>
    </div>
  );
}

export default IniciarSesionForm;